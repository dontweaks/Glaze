...
namespace glaze::ecs {
	...
	struct Archetype {
		...
		[[nodiscard]] std::pair<std::optional<Entity>, TableRow> remove_entity(const ArchetypeRow row) noexcept {
			...
			return { is_last ? std::nullopt : std::make_optional(m_entities[index].entity), table_row };
		}
